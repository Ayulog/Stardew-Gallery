...
基线：2.5.0 `c4435f0`。当前开发版：2.6.0-dev.3。dev.1 的 Mud 组合已获用户正常反馈；Portraiture专项开发交付，待实际包实测。

## 当前范围调整

用户决定美化协调器留待以后，不管理其他美化包的启用或覆盖。用户随后明确授权继续Portraiture正确读取；画廊换肤仍后置。当前只做画廊能够负责的读取、异常隔离及显示回退。
