...
基线：2.5.0 `c4435f0`。最终版本：2.6.0。用户已明确确认单个美化模组支持正常，并授权main、Release、Nexus发布；多美化互相覆盖请用户自行实测。下文dev.1至dev.4保留各阶段原始证据，阶段性待实测说明以本段最终验收为准。

## 当前范围调整

收尾决定：婚礼、节日、夜间事件、影院等专门流程不加入画廊，相关调研归档为范围说明。本轮不再扩大第三方条件/未生效CP目录或专用主题开发。保留已有好感相册、普通剧情/前置查询、截图封面与角色外观兼容，转入最终验收和发布准备。
...
- 已确认的缓存恢复缺陷在dev.4修复，定向复查未发现新的确定性程序缺陷。四份相关隔离报告findings为0，且DLL SHA256全部与当前安装一致：`FF8B9B8B58A3EBDFAD836BF9115046DB42BC7EED5A3329AD81D49DD8E6B2329C`。
- README补齐可选美化框架安装/使用和明确的剧情范围；第三方声明补齐美化集成来源，并修正错误地将2.6开发包源码指向v2.5.0的文字。此轮只改文档，不重新构建或重跑无变化的程序检查。
- 2.6.0发布摘要已准备于 `docs/RELEASE_2.6.0_NOTES.md`，仍是待发布材料。main/正式Release当前为2.5.0，未从旧版发布授权推定新的发布操作。
- 最近日志最后写入12:27:56，记录仍为dev.3；dev.4在12:45安装。没有dev.4的后续运行记录，因此最后修复的实机确认仍待完成；不以日志中的旧Emily警告判定dev.4失败。
